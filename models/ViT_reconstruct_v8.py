...
@article{svantesson2021virtual,
  title={Virtual EEG-electrodes: Convolutional neural networks as a method for upsampling or restoring channels},
  author={Svantesson, Mats and Olausson, H{\aa}kan and Eklund, Anders and Thordstein, Magnus},
...
class ViT_reconstruct_v8(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        # Decoder
        self.up1 = nn.UpsamplingBilinear2d((129, 14))

        self.dec_block1 = nn.Sequential(
            nn.ConvTranspose2d(768, 256, (1, 3), stride=(1, 2), padding=(0, 1)),
            nn.InstanceNorm2d(256),
            nn.ReLU(True)
        )

        self.dec_block2 = nn.Sequential(
            nn.ConvTranspose2d(256, 128, (1, 3), stride=(1, 2), padding=(0, 1)),
            nn.InstanceNorm2d(128),
            nn.ReLU(True)
        )

        self.dec_block3 = nn.Sequential(
            nn.ConvTranspose2d(128, 64, (1, 3), stride=(1, 2), padding=(0, 1)),
            nn.InstanceNorm2d(64),
            nn.ReLU(True)
        )

        self.dec_block4 = nn.Sequential(
            nn.ConvTranspose2d(64, 1, (1, 3), stride=(1, 2), padding=(0, 1)),
            nn.InstanceNorm2d(1),
            nn.ReLU(True)
        )

        self.up2 = nn.UpsamplingBilinear2d((129, 500))  # fixed output size
        self.tanh = nn.Tanh()
    ...
    def forward(self, x):
        ...
        x_reconstructed = self.up1(reshaped_features)
        x_reconstructed = self.dec_block1(x_reconstructed)
        ...
        x_reconstructed = self.up2(x_reconstructed)
